...
<div class="bg-primary-surface border border-primary-border rounded-3xl p-8 max-w-2xl">
    <h2 class="text-2xl font-headline font-bold text-white mb-8">Record Payment</h2>
    
    <form method="POST" action="{{ route('gym.payments.store') }}" class="space-y-6">
        @csrf
        ...
        <div>
            ...
            <select name="gym_package_id" id="package-select" required class="w-full bg-black/20 border border-primary-border rounded-xl py-3 px-4 text-white focus:outline-none focus:border-primary-lime transition-all">
                ...
                <option value="{{ $pkg->id }}" data-price="{{ $pkg->price }}" {{ old('gym_package_id') == $pkg->id ? 'selected' : '' }}>
        ...
        <div>
            ...
            <input type="number" name="amount_paid" id="amount-paid" value="{{ old('amount_paid') }}" required min="0" step="0.01" class="w-full bg-black/20 border border-primary-border rounded-xl py-3 px-4 text-white focus:outline-none focus:border-primary-lime transition-all">
        </div>
        
        <div>
            <label class="block text-sm text-on-variant mb-2">Start Date *</label>
            <input type="date" name="start_date" value="{{ old('start_date', now()->toDateString()) }}" required class="w-full bg-black/20 border border-primary-border rounded-xl py-3 px-4 text-white focus:outline-none focus:border-primary-lime transition-all">
        </div>
...
document.getElementById('package-select').addEventListener('change', function() {
    ...
    if (price) {
        document.getElementById('amount-paid').value = price;
    }
});
</script>
@endpush
